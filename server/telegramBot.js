...
export function startBot({ events } = {}) {
  ...
  if (events) {
    events.on('trade-resolved', ({ coin, win, pnl, source, sourceLabel, walletId }) => {
      const w = getState().wallets.find((x) => x.id === walletId);
      const wlabel = w ? `${w.label}` : '';
      const emoji = win ? '✅' : '❌';
      broadcast(`${emoji} *${escMd(sourceLabel || source)}* ${coin} ${win ? 'WON' : 'LOST'} ${fmtPnl(pnl)}${wlabel ? ` · ${escMd(wlabel)}` : ''}`);
    });
