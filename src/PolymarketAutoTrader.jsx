...
export default function App() {
  ...
  function runSimTick() {
    ...
    for (const s of liveStrategies) {
      ...
      const candle = candleInfo(Date.now(), s.timeframe);
      if (s.lastCandleKey === candle.key) continue;
      const baseProb = 0.45;
        ...
        prev.map((x) =>
          x.id === s.id
            ? {
                ...
                lastCandleKey: candle.key,
                lastCandleLabel: candle.label,
              }
            : x
        )
      );
    ...
    for (const c of liveCopies) {
      ...
      if (c.lastCandleKey === candle.key) continue;
      const polyPrice = livePoly[coin]?.yesCents ?? 50;
      ...
      <div className="panel">
        ...
        <div className="log-list">
          ...
          {filtered.map((l) => {
            ...
            const candleLabel =
              l.candleLabel ||
              (l.ts && l.timeframe && l.timeframe !== '—' ? candleInfo(l.ts, l.timeframe).label : '');
            const leftText =
              Number.isFinite(l.secondsBeforeClose)
                ? `${l.secondsBeforeClose}s`
                : (l.candleEnd ? `${Math.max(0, Math.floor((l.candleEnd - l.ts) / 1000))}s` : '');
            return (
              <div key={l.id} className={`log-row status-${(l.status || '').toLowerCase()}`}>
                <span className="log-time mono small">
                  <span>{fmtTime(l.ts)}</span>
                  {candleLabel && <span className="log-candle">{candleLabel}</span>}
                </span>
                <span>
                  {w ? (
                    <span
                ...
                <span className="mono small">{Number.isFinite(l.exitPrice) ? fmtCents(l.exitPrice) : '—'}</span>
                <span className="mono small" title="Seconds before candle close when trade was placed">{leftText || '—'}</span>
                ...
                <span className={`mono small ${l.pnlPct >= 0 ? 'pos' : 'neg'}`}>{l.pnlPct != null ? fmtPct(l.pnlPct) : ''}</span>
...
.log-head, .log-row {
  display: grid;
  grid-template-columns: 82px 90px 50px 50px 30px 52px 56px 56px 44px 60px 60px 52px 70px 1fr;
  gap: 8px;
  padding: 7px 10px;
  align-items: center;
  font-size: 11px;
...
@media (max-width: 900px) {
  ...
  .log-head, .log-row {
    grid-template-columns: 76px 70px 38px 42px 24px 40px 48px 48px 38px 52px 52px 44px 60px 1fr;
    font-size: 10px;
  }
...
@media (max-width: 600px) {
  ...
  .log-head, .log-row {
    grid-template-columns: 82px 70px 42px 52px 52px 42px 58px 50px;
  }
  .log-row > :nth-child(3),
  ...
  .log-row > :nth-child(10),
  .log-head > :nth-child(10) { display: none; }
  .log-row > :nth-child(13),
  .log-head > :nth-child(13) { display: none; }
  .log-source { display: none; }
}
`;
